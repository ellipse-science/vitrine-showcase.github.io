...
// Le cœur de la sélection de la Une vit désormais dans un module PUR, pour que
// le Worker puisse l'appeler avant le build (aws-refiners#490). Il est
// ré-exporté ici : tout ce qui importait ces symboles depuis ce module
// continue de fonctionner, et surtout site et Worker appellent LA MÊME
// fonction — jamais une copie (vitrine-showcase#259).
import {
  type RawEvent,
  type Story,
  type HeroSelection,
...
export const loadHeadlineEvents = cache(async (editionKey?: string, opts?: { classement?: number }): Promise<HeadlineData | null> => {
  ...
  const toUneEvent = (s: Story): UneEvent => {
    ...
    // Un média qui a porté l'histoire dans les 24 h mais pas dans CE bloc
    // compte dans « 3/6 en parlent » (media_ids_24h) : le commentaire doit
    // donc le citer aussi, sinon le post promet trois médias et n'en montre que
    // deux (La Presse, 17-09 à 20h : Une de mercredi soir). On complète depuis
    // articles_24h, un article par média, chaque ligne gardant sa signature ;
    // le bloc courant garde la priorité.
    try {
