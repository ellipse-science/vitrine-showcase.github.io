...
| Résultats électoraux (gagnant, voix, avance, participation), 43e législature | `vitrine/scripts/social/donnees/resultats-elections.json`, tiré des données ouvertes d'Élections Québec par `scripts/social/resultats-elections.ts` | COPIE LOCALE, provisoire | ajouté pour les cartes de député (2026-09-23) ; devrait rejoindre la dimension `dim_qc_parliament` (`pplmatch`). Méthode : `docs/reference/cartes-deputes.md` |
| Fonctions parlementaires datées, porte-parole et vis-à-vis, 43e législature | `vitrine/scripts/social/donnees/fonctions-deputes.json`, tiré des fiches assnat.qc.ca par `scripts/social/fonctions-deputes.ts` | COPIE LOCALE, provisoire | la colonne `functions` de `dim-qc-parliament-members-staging` (aws-infra) est un instantané du 2024-06-21, inutilisable ; à verser dans `dim_qc_parliament` (`pplmatch`) |
| Indemnités des député·e·s (base 2022-2026, barème des fonctions) | constantes `BASES` et `BAREME` de `vitrine/scripts/social/fonctions-deputes.ts`, relevées sur assnat.qc.ca (versions archivées) et L.Q. 2023, c. 14 | source unique, locale | ne pas retaper ailleurs ; dériver de ces constantes ou de la future table |
