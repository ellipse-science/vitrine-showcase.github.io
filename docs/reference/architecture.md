...
**Note:** `scripts/fetch_data.R` currently appends `[skip ci]` to its commit messages to keep the GitHub Pages deploy from rebuilding on every 4h refresh. Whether to remove this is tied to the hosting decision; see [`docs/cloudflare-pages-migration.md`](../cloudflare-pages-migration.md).
