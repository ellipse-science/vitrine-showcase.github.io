...
6. **Impact méthodologie obligatoire sur chaque PR.** La page Méthodologie (`public/methodologie/index.html`) et les docs vivantes du pipeline (`public/docs/horaire-refiners-2026.html`, `public/docs/workflow-vitrine-2025-swimlanes.html`) ne doivent JAMAIS être périmées. Toute PR (de ce repo ou des repos AWS) qui change un calcul, un seuil, un horaire, la collecte ou une représentation met le texte à jour ou déclare explicitement l'absence d'impact — section « Impact méthodologie » du template de PR, vérifiée par le check `garde-metho`. Guide complet (mapping sections ↔ code, procédure, timing) : [`.claude/skills/synchro-methodologie/SKILL.md`](./.claude/skills/synchro-methodologie/SKILL.md). Timing : pour un changement aws-refiners/aws-infra, la PR métho se merge quand le changement est **déployé** (corollaire de FAIT vs VISION).

7. **Attribution humaine des commits — aucune IA en auteur ni co-auteur.** Les commits sont attribués à la personne humaine qui a fait le travail. Jamais de trailer `Co-Authored-By` pointant vers une IA (`noreply@anthropic.com`, Copilot, etc.), jamais d'auteur/committer IA (vérifiez `git config user.name` / `user.email` dans vos environnements d'agents). Pourquoi : GitHub crédite les co-auteurs dans le graphe Contributors — le compte `claude` y apparaissait comme 2e contributeur du repo. Appliqué mécaniquement : `includeCoAuthoredBy: false` dans `.claude/settings.json` (Claude Code) + check CI `garde-attribution` sur chaque PR. Contexte : [issue #235](https://github.com/ellipse-science/vitrine-showcase.github.io/issues/235).
